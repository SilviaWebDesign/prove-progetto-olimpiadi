<script>
  ...
  /** @param {number} fade */
  function updateMountainFade(fade) {
    for (const { material, originalColor } of mountainMaterials) {
      if ('color' in material && material.color instanceof THREE.Color) {
        material.color.copy(originalColor).lerp(whiteColor, fade);
      }
      material.opacity = 1 - fade;
    }
    if (snowMountainModel) {
      snowMountainModel.visible = fade < 1;
    }
  }
  ...
  onMount(() => {
    ...
    const animate = () => {
      ...
      const tZoom = easeInOutCubic(clamp(progress / TEXT_END, 0, 1));
      ...
      if (progress <= TEXT_END) {
        updateMountainFade(0);
        if (sceneFog) sceneFog.density = 0.045;
      } else {
        const fade = smoothstep(TEXT_END, MOUNTAIN_FADE_END, progress);
        updateMountainFade(fade);
        if (sceneFog) sceneFog.density = lerp(0.045, 0.12, fade);
      }
